(function() {
	...
	schedule.on('declared', function() {
		//These connect the json in the code with the things shown in dashboard.
		var currentRun = nodecg.Replicant('currentRun');
		var runInfoName = document.querySelector('label-value[label="Name"]');
		var runInfoRoles = document.querySelector('label-value[label="Roles"]');
		var runInfoStartTime = document.querySelector('label-value[label="StartTime"]');
